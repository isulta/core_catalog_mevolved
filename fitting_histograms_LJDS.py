...
from itk import hist, h5_read_dict, gio_read_dict, loadpickle, plt_latex, periodic_bcs, many_to_one, pickle_save_dict
...
M1, M2 = SHMLM.PARTICLES100MASS*10, 10**16
# M1, M2 = 10**12, 10**13
# M1, M2 = 10**13, 10**14
# M1, M2 = 10**14, 10**15
...
#         cores_tree, cc_filtered, nHalo = generate_cores_kdtree(M1, M2, s1=False, disrupt=None, onlyFiltered=False, giveFragmentsFofMass=False)
        ...
        if logmOverMFlag:
            ...
            r = (-3, 0)
            shmf = sh['subhalo_mass'][sh_mask][sh100mask] / sh['M'][sh_mask][sh100mask]
            sh_xarr, sh_cnts = hist(np.log10(shmf), bins=100, normed=True, normBinsize=normBinsize, normCnts=normCnts, normLogCnts=normLogCntsFlag, normScalar=normScalarVal, plotFlag=plotFlag, label='subhalos', alpha=1, range=r)

            cmf = cc_filtered[m_evolved_col(A, zeta)][cores100mask] / cc_filtered['M'][cores100mask]
            cores_xarr, cores_cnts = hist(np.log10(cmf), bins=100, normed=True, plotFlag=plotFlag, label='cores', alpha=1, range=r, normScalar=normScalarVal, normCnts=normCnts, normBinsize=normBinsize, normLogCnts=normLogCntsFlag)
        else:
            r = (9, 15)
...
def plots100PartResidual(dir, residualRange, M1, M2, phaseMergedFlag, normCnts=False, saveFlag=False):
    ...
    for i in range(len(A_arr)):
        for j in range(len(zeta_arr)):
            ...
            R[i,j] = subhalo_core_mass_plots(A, zeta, M1, M2, plotFlag=False, residualRange=residualRange, normLogCntsFlag=False, logmOverMFlag=True, phaseMergedFlag=phaseMergedFlag, normCnts=normCnts, normScalar=True, normBinsize=True)
            Rlog[i,j] = subhalo_core_mass_plots(A, zeta, M1, M2, plotFlag=False, residualRange=residualRange, normLogCntsFlag=True, logmOverMFlag=True, phaseMergedFlag=phaseMergedFlag, normCnts=normCnts, normScalar=True, normBinsize=True)
            #print R[i,j]
            assert np.isfinite(R[i,j])
    plt.figure()
    plt.pcolormesh(zeta_arr, A_arr, R, cmap=plt.cm.jet)
    cb = plt.colorbar()
    cb.set_label('2-norm relative error')
    if saveFlag:
...
if __name__ == "__main__":
    ...
    def cc_filtered_gen(lM):
        M1, M2 = 10**lM, 10**(lM+massbinsize)
        return generate_cores_kdtree(M1=M1, M2=M2, s1=False, disrupt=None, onlyFiltered=True, giveFragmentsFofMass=False)
