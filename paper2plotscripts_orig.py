...
def resolution_tests(cc_HM, sh_HM, centrals_mask_HM, cc_SV, sh_SV, centrals_mask_SV, mplot=False, A=None, zeta=None, ccMvar='M', fixedAxis=True):
...
def cosmology_tests(cc_SV, centrals_mask_SV, cc_AQ, centrals_mask_AQ, r=(-4.0,0.), bins=20, A=None, zeta=None, mlim=0, mplot=False):
    alpha = 1
    ...
    for logM1, axr, marker in zip((12, 13, 14), (ax1,ax2,ax3), ('o', 'D', 's')):
        ...
        for cc, centrals_mask, label in zip([cc_SV, cc_AQ], [centrals_mask_SV, centrals_mask_AQ], ['SV', 'AlphaQ']):
            ...
            errorbar(ax, x, y, yerr=yerr_log, label='Cores {} {}'.format(logM1, label), marker=marker, alpha=alpha)
...
def sigma1plots(cc, sh, centrals_mask, label, rdict, M1dict, M2dict, ReducedChi2dict, bins=20, mlim=0, mplot=False, outfile=None, avgchi2=False):
    alpha = 1.0
    ...
    for Mlabel, ax in zip( (12, 13, 14), (ax1, ax2, ax3) ):
        ...
        errorbar(ax, x, y, yerr=yerr_log, label='Cores (avg. best-fit parameters)', c='k', alpha=alpha )
        print()
...
def subhalo_core_match(M1, M2, cc, centrals_mask, sh, step, mlim=OBJECTMASSCUT['SV'], SHMLM=SHMLM_SV, A=AFID, zeta=ZETAFID, s1=False, mostMassiveCore=False, assert_nH=True):
...
def histogram_matched_m(matched_ifhm_cores, matched_m_sh, matched_m_cores, M1, M2, bins=100, r=(0.0, 2.1), alpha=0.3, density=True, legend=False):
    plt.figure()
